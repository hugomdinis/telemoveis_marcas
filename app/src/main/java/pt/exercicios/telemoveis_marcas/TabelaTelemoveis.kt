...
class TabelaTelemoveis(db: SQLiteDatabase) : TabelaBD(db, NOME_TABELA) {
    ...
    override fun cria() {
        db.execSQL("CREATE TABLE $NOME_TABELA ($CHAVE_TABELA,$CAMPO_MODELO TEXT NOT NULL, $CAMPO_INFORMACAO TEXT NOT NULL, $CAMPO_ANO DATE NOT NULL, $CAMPO_FK_MARCA INTEGER NOT NULL, FOREIGN KEY($CAMPO_FK_MARCA) REFERENCES ${TabelaMarca.NOME_TABELA}(${BaseColumns._ID}) ON DELETE RESTRICT)")
    }
